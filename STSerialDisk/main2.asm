...
.equ bcb_data,		0x4B2														| Pointer to a Buffer Control Block for sector data
.equ bcb_fat,		0x4B6														| Pointer to a Buffer Control Block for FAT data
...
|
allocate_buffers:
	|move.w	#0x2000, d0			| Get bytes per sector from BPB
	|lsl		#2,d0				| Sector size * 4 for the 4 buffers

	|Malloc 	d0					| Allocate required RAM for sector_size * 4 buffers
	|lea		d0,a2				| Store address of new disk buffer
	move.l	#disk_buffer,d0

	| Data buffers

	move.l 	#bcb_data,a1		| Load address of first data BCB pointed to at 0x4B2
	move.l	(a1),a1
	move.l	a2,16(a1)			| Set buffer address to new allocated area of RAM
	move.w	#-1,4(a1)			| Mark current buffer contents as out-of-date ("dirty")

	add.l	#0x2000,d0			| Offset to next sector area in buffer

	move.l	(a1),a1				| Update a1 with address of next BCB pointed to by this BCB
	move.l	d0,16(a1)			| Set buffer address to new allocated area of RAM
	move.w	#-1,4(a1)			| Mark current buffer contents as out-of-date ("dirty")
	...
	move.l 	#bcb_fat,a1		| Load address of first FAT BCB pointed to at 0x4B6
	move.l	(a1),a1
	move.l	d0,16(a1)			| Set buffer address to new allocated area of RAM
	move.w	#-1,4(a1)			| Mark current buffer contents as out-of-date ("dirty")
...
disk_buffer:
	|ds.l	1
	|ds.b	0x8000	| 32768 bytes = 2 sectors of 2 * 8192 bytes
